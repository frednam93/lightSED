...
def get_configs(saved_model):
    if saved_model is not None:
        config_dir = os.path.join(saved_model, "config.yaml")
    else:
...
def get_mt_datasets(configs, train_cfg):
    ...
    synth_train_tsv = dataset_cfg["synth_train_tsv"]
    ...
    synth_valid_dir = dataset_cfg["synth_val_folder"]
    synth_valid_tsv = dataset_cfg["synth_val_tsv"]
    synth_valid_df = pd.read_csv(synth_valid_tsv, sep="\t")
    synth_valid_dur = dataset_cfg["synth_val_dur"]

    strong_train_dataset = StronglyLabeledDataset(synth_train_df, dataset_cfg["synth_train_folder"], False, encoder)
...
def get_models(configs, train_cfg, logger):
    #student model
