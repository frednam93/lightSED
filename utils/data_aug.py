...
def data_augs(train_cfg, mels, labels, weak_labels=None, mask_strong=None, mask_weak=None):
    ### Data augmentation methods that change labels
    # frame_shift
    if train_cfg["frame_shift"]:
        mels, labels = frame_shift(mels, labels, train_cfg["net_pooling"])

    if weak_labels is not None:
        # mix-up
        if train_cfg["mixup_rate"] > torch.rand(1).item():
            # weak data mixup                      # strong_masked feature size = [bs_strong, freq, frames]
            mels[mask_weak], weak_labels = mixup(mels[mask_weak], weak_labels,
        ...
        # mix-up
        mels, labels = mixup(mels, labels, mixup_label_type=train_cfg["mixup_type"])

        # time masking
        if train_cfg["time_masking"]:
            mels, labels = time_mask(mels, labels, train_cfg["net_pooling"], mask_ratios=train_cfg["time_mask_ratios"])

    ### Data augmentation methods that do not change labels
    # data augmentations without affecting labels
    mels_1, mels_2 = feature_transformation(mels, **train_cfg["transform"])

    return mels_1, mels_2, labels, weak_labels
...
def time_mask(features, labels=None, net_pooling=None, mask_ratios=(5, 20)):
...
def feature_transformation(features, n_transform, choice, filter_db_range, filter_bands, min_bw, filter_type,
                           freq_mask_ratio, noise_snrs):
    if n_transform == 2:
        # two differently augmented features
        feature_list = []
        for _ in range(2):
            features_temp = features
            if choice[0]:
                features_temp = filt_aug(features_temp, db_range=filter_db_range, n_band=filter_bands,
                                         min_bw=min_bw, filter_type=filter_type)
            if choice[1]:
        ...
        # two features with same augmentations
        if choice[0]:
        ...
        # two features without augmentation
        return [features, features]


def filt_aug(features, db_range=[-6, 6], n_band=[2, 5], min_bw=4, filter_type="step"):
